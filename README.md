...
ArangoDB is a native multi-model database with flexible data models for document, graphs, and key-values.
ArangoRB would like to be a Ruby Gem to use ArangoDB with Ruby. ArangoRB is based on the HTTP API of ArangoDB.
...
We can even do some combinations:

A.out(a)[0].to.out(a)[0].to is E since:
A.out(a) is [aa]
A.out(a)[0] is aa
A.out(a)[0].to is B
