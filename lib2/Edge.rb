...
class Arango::Edge < Arango::Document
  def initialize(collection:, graph:, database:, client:, key: nil, body: {}, from: nil, to: nil) # TESTED
    satisfy_class?(database, "database", [Arango::Database, String])
    satisfy_class?(database, "graph", [Arango::Graph, String])
    satisfy_class?(database, "collection", [Arango::Collection, String])
    ...
    if collection.is_a?(String)
      @collection = Arango::Collection.new(collection: collection, database: @database, client: @client)
    else
      @collection = collection
    end
    if graph.is_a?(String)
      @graph = Arango::Graph.new(graph: graph, database: @database, client: @client)
    else
      @graph = graph
    end
